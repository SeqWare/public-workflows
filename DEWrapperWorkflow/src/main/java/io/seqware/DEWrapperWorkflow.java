    ...
    public void buildWorkflow() {
        ...
        for (int i = 0; i < analysisIds.size(); i++) {
            ...
            if (localFileMode) {
              ...
              .addArgument("mkdir -p " + analysisIds.get(i) + " && sudo ln "+bams.get(i)+" `pwd`/"+SHARED_WORKSPACE+"/"+analysisIds.get(i)+"/ && sudo ln "+bams.get(i)+".bai `pwd`/"+SHARED_WORKSPACE+"/"+analysisIds.get(i)+"/");
            } else {
              downloadJob
                      .getCommand()
                      .addArgument(
