...
#
# Last Modified: 2014-05-13, Status: basically working
...
# print STDERR "Skipping metadata download for this run.\n" unless $skip_down;
...
# output data table
open my $FH, '>', 'data_freeze_alignment_report__' . $timestamp . '.tsv' or die "Could not open table for writing: $!";
...
print STDERR "WARNING: Logged $error_log errors in error_log.txt stamped with $timestamp\n" if $error_log;
...
sub schedule_samples {
    ...
    # These columns were requested by Lincoln, I added three more after further discussions with Lincoln
    # and Brian.  Oh, wait a second, when he says GNOS endpoint he means the full analysis URI thing, so I can
    # delete one of my columns
    # Oh, I see, Study is a field in Hardeep's master list so I will make a hash of those
    ...
    process_specimens( $single_specimens );
    process_specimens( $two_specimens );
    process_specimens( $many_specimens );
            ...
            foreach my $specimen (keys %{$sample_info->{$project}{$donor}}) {
                ...
                foreach my $sample ( keys %{$sample_info->{$project}{$donor}{$specimen}} ) {
                    foreach my $alignment ( keys %{$sample_info->{$project}{$donor}{$specimen}{$sample}} ) {
                        ...
                        # my $endpoint = $sample_info->{$project}{$donor}{$specimen}{$sample}{$alignment}{analysis_url};
                        ...
                        my $analysis_id = $sample_info->{$project}{$donor}{$specimen}{$sample}{$alignment}{analysis_id};
     	                $aligns->{$alignment} = 1;
                            ...
                            # actually, in this script we are only counting aligned files
              	            if ( $num_bam_files == 1 ) {
...
sub log_error {
    my ($mesg) = @_;
    open my ($ERR), '>>', 'error_log_' . $timestamp . '.txt' or die;
    print $ERR "$mesg\n\n";
    close $ERR;
    $error_log++;
} # close sub
